window.__define_resource && __define_resource("LGN00105","LGN00010","LGN00202","LGN00009","LGN00006","MSG00254","MSG01136","LGN00013","LGN00014","LGN00015","LGN00016","LGN00113","LGN00114","LGN00115","LGN00203","LGN00012");
/**************************************************************************************************** 
1. Create Date : 20??.??.?? 
2. Creator     : unknown
3. Description : CS Login 메인 화면 js
4. Precaution  : 
...
function onloadHandler() {    
    ...
    //_loginErrorMsg = String.format("{0}</ br>{1}", ecount.resource.LGN00010, ecount.resource.LGN00202);
...
function excuteLogin() {
    ...
    _comCode = document.getElementById('cs_com_code').value.trim();
    _id = document.getElementById('id').value.trim();
    $("#cs_com_code").val(_comCode);
    $("#id").val(_id);
    ...
    if (validationCheck()) {
        ...
        document.getElementById('cs_com_code').value = document.getElementById('cs_com_code').value.replace(/[&|?=]/g, function (match) {
            switch (match) {
                case "&":
                    return "＆";
                case "|":
...
function validationCheck() {
    ...
    if (document.getElementById('cs_com_code').value == "")
    ...
    if (inputNode[0].className == 'input-group has-error' || inputNode[1].className == 'input-group has-error' || inputNode[2].className == 'input-group has-error') {
        ...
        if (inputNode[0].className == 'input-group has-error') {
            document.getElementById('cs_com_code').focus();
            if (document.getElementById('passwd').value == "" || fmt1.exec(document.getElementById('passwd').value))
                inputNode[2].className = 'input-group has-error';
        }
...
// get zone info
function getZone() {
    var comCode = document.getElementById('cs_com_code').value;
    var param = {
...
function execLoginProc() {
    ...
    //$("#cs_com_code").val($("#com_code").val());
    $("#com_code").val(_comCode);
    $("#zone").val(_zone);
    $("#domain").val(_domain);

    $("#loginForm").attr("action", host + "/ECERP/LOGIN/CSLoginProcessor").submit();
}
...
function closeErrorPopup() {
    ...
    document.getElementById("cs_com_code").value = "";
...
function searchPasswordPopup() {
    var width = 835;
    var height = viewBag.lanType != "es" ? 375 : 420;
    var left = (screen.width / 2) - (width / 2);
    var top = (screen.height / 2) - (height / 2);

    window.open("/ECERP/Popup.Common/PasswordSearch?DOMAIN_FLAG=C&lan_type=" + viewBag.lanType, "searchPassword", "scrollbars=no,resizable=yes,copyhistory=no,scrollbars=no,width=" + width + ",height=" + height + ",top=" + top + ",left=" + left);
}
...
function UpGradeBrowserNoticePopup(type) {
    ...
    var cookieName = type == "browser" ? "UpGradeBrowserNoticeCS" : "UpGradeOSNoticeCS";
...
function OpenPageForGMC(url, name, width, height) {
    ...
    hostUrl = '';
    debugger;
    if (window.location.host.indexOf("tcs") > -1 || window.location.host.indexOf("test") > -1) {
        domain = "test.ecount.com";
    } else if (window.location.host.indexOf("zcs") > -1 || window.location.host.indexOf("hcs") > -1 || window.location.host.indexOf("acs") > -1) {
...
function writeCallBack() {
    ...
    ecount.common.api({
        url: ecount.common.makeGmcHost() + "/Service/CustomerCenter/CheckCallBackTime?",
        success: function (result) {
            if (result.Status != "200") {
                alert(result.Error);
            } else {
                if (result.Data.IsWritePossibleTime) {
                    //팝업창 오픈
                    OpenPageForGMC("/ECERP/ECU/ECU005M_NoAuth", "callback", "800", "800");
                } else {
                    alert(this.viewBag.LGN00203);
                }
            }
        },
    });
...
function lanTypeChange(lan) {
    document.location.href = "/ECERP/Login/CSLogin?lan_type=" + lan;
}